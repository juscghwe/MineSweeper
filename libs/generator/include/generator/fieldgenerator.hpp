...
class FieldGenerator
{
  public:
     ...
     * @return `std::unique_ptr<Utility::FieldVector>` A unique pointer to the generated `FieldVector` representing the minefield.
     * @details This method randomly places mines in the minefield and calculates the adjacent mine counts.
     */
    std::unique_ptr<Utility::FieldVector> generateField();
  ...
  private:
    ...
    std::unique_ptr<Utility::FieldVector>
        fieldGrid_;  ///< `unique_ptr<Utility::FieldVector>` Unique pointer to the minefield grid.
     ...
     * @return `std::set<Utility::PositionStruct>` A set of objects representing 2D coordinates of the mines.
     */
    std::set<Utility::PositionStruct> minePlacementTwoD(const std::set<int>& mineFieldOneD) const;

    /**
     * @private
     * @brief Places mines in the field grid based on the provided 2D positions.
     * @param mineFieldTwoD `std::set<Utility::PositionStruct>` A set indicating where to place the mines.
     */
    void placeMines(const std::set<Utility::PositionStruct>& mineFieldTwoD);

    /**
     * @private
     * @brief Calculates the number of adjacent mines for a given position in the minefield.
     * @param position `Utility::PositionStruct` The field for which to calculate adjacent mines.
     */
    void calculateAdjecentMines(const Utility::PositionStruct& position);
};
